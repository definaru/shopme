...
{
    const ClassStyle = 'text-gray-600 hover:text-pink-600 hover:bg-gray-50 flex rounded-md items-center w-full px-2 py-2 text-sm cursor-pointer'
    const list = ListLang()
                    ...
                    <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                        <div className="relative grid gap-1 bg-white p-2 grid-cols-1">
                            {list.map((item) => (
                                ...
                                >
                                        <img src={item.flag} alt={item.name} className="w-6 h-4 mr-2 border" />
                                        {item.name}
                                </a>
                            ))}
                        </div>

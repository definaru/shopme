...
export function WebApps() 
{
    ...
    return (
        <Menu as="div" className="relative">
            <div className="bg-white hover:bg-gray-50 rounded-full w-10 h-10 cursor-pointer grid place-items-center">
                <Menu.Button className="focus:outline-none">
                    <FiGrid className="w-5 h-5 text-gray-400 hover:text-gray-500" />
                </Menu.Button>
            </div>
            <Transition
                as={Fragment}
                ...
                <Menu.Items className="absolute right-0 w-80 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    ...
                    >
                        <div className="p-0">
                            {onData?.map((item, i) => (
                            <Menu.Item key={i}>
                                <a href={item.href} className="flex items-center rounded bg-white hover:bg-gray-50 px-6 py-3">
                                    <div>
                                        <img 
                                            src={item.image}
                                            alt={item.header} 
